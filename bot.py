...
BOT_NAME = os.getenv('BOT_NAME', 'Svm-v9')
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            ...
            ssh_host_port = await create_port_forward(user_id, container_name, 22, self.node_id)
            ...
            if ssh_host_port:
                ...
                add_field(dm_embed, "🔑 SSH Login", f"**IP:** `{public_ip}`\n**Port:** `{ssh_host_port}`\n**User:** `root`\n**Password:** `{root_password}`\n\n```ssh root@{public_ip} -p {ssh_host_port}```", False)
            else:
                add_field(dm_embed, "🔑 Root Password", f"`{root_password}`\n⚠️ Port forwarding for SSH failed — ask an admin to run `{PREFIX}repair-ports {container_name}`.", False)
...
class ReinstallOSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            ...
            ssh_host_port = await create_port_forward(self.owner_id, self.container_name, 22, self.node_id)
            ...
            try:
                ...
                if ssh_host_port:
                    ...
                    add_field(dm_embed, "🔑 Root Password", f"`{root_password}`\n⚠️ Port forwarding for SSH failed — ask an admin to run `{PREFIX}repair-ports {self.container_name}`.", False)
